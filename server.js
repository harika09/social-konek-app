      ...
      if (err instanceof multer.MulterError) {
        ...
        if (!title || !content || !req.file) {
          ...
          if (req.file) {
            ...
            post.imgKey =
              req.file.key; /* AWS S3 key is needed to locate the file inside the bucket */
  ...
  if (req.isAuthenticated()) {
    ...
    User.findOne({ _id: req.user.id }, function (err, data) {
      if (err) {
        ...
        if (data.likes.includes(requestedPostId)) {
          /* Check if user like array contains post ID */
  ...
  if (req.isAuthenticated()) {
    ...
    User.findOne({ _id: requestedUserId }, function (err, user) {
      if (err) {
        ...
        res.render("settings", {
          user: user,
          limit: req.flash("limit"),
          empty: req.flash("emptyFields"),
          message: req.flash("invalid"),
        });
      }
    });
...
app.post("/update", urlencodedParser, (req, res, next) => {
  if (req.isAuthenticated()) {
    Upload(req, res, function (err) {
      let { userId, email, username, name, firstname, lastname, avatar } =
        req.body;
      const requestedUserId = req.user.id;
                ...
                User.updateOne(
                  ...
                  function (err) {
                    if (err) {
                      ...
                      Post.updateMany(
                        { _id: requestedPostId },
                        { $set: { username: username, avatar: userAvatar } },
                        { multi: true },
                        function (err, data) {
                          if (err) {
                            res.send(err);
...
app.post(
  "/login",
  urlencodedParser, //Handle unauthorized request
...
app.post("/register", urlencodedParser, (req, res) => {
  ...
  const image =
    "https://img.favpng.com/25/15/22/computer-icons-user-profile-icon-design-png-favpng-0VbLzJjbKEpnLF68K4UituW9P.jpg";
...
app.use(function (req, res) {
  if (req.isAuthenticated()) {
    res
      .status(404)
      .render("404"); /* When page(s) is not found redirect to error page 404 */
  } else {
    res.redirect("/");
  }
});
